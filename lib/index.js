...
class Generator extends EventEmitter {
  ...
  async readConf(env) {
    ...
    if (env === 'dev') this.conf.mode = 'hash';
  ...
  async preRender(data) {
    const { env, mode } = this.conf;
    if (env !== 'build' || mode === 'hash') return;
  ...
  async build(env = 'build') {
    await this.emitAsync('buid', this);
    await this.readConf(env);
  ...
  async watchPlugins() {
    const files = this.plugins.map(plugin => `${plugin.location}/**/*.*`);
