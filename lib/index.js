...
const builtInPlugins = [
  ...
  path.resolve(__dirname, '../plugins/anchor'),
  path.resolve(__dirname, '../plugins/toc'),
];
...
class Generator extends EventEmitter {
  ...
  async parseGroup(group) {
    ...
    group.docs = (await utils.batchExec(docFiles, item => this.parseDoc(item)))
      .filter(doc => !!doc);
    return group;
  }
  ...
  async parseLocale(locale) {
    ...
    delete locale.docs;
    const docs = (await utils.batchExec(docFiles, item => this.parseDoc(item)))
      .filter(doc => !!doc);
    docs.forEach(doc => {
  ...
  async generate(data) {
    ...
    if (this.env !== 'build' || !mode || mode === 'hash') return;
    const { generate } = require('./generator');
    await generate(this, data);
  }
  ...
  async writeAsstes() {
    ...
    await utils.batchExec(this.plugins, item => this.writePluginAssets(item));
    await this.injectPluginAsstes();
  }

  getPluginAssets() {
    if (this.__pluginAssets) return this.__pluginAssets;
    const styles = this.assets.styles.sort(utils.strEqual)
      .map(url => `<link href="./${url}" rel="stylesheet" />`)
      .join('') + '</head>';
    const scripts = this.assets.scripts.sort(utils.strEqual)
      .map(url => `<script src="./${url}"></script>`)
      .join('') + '</body>';
    this.__pluginAssets = { styles, scripts };
    return this.__pluginAssets;
  }

  async injectPluginAsstes() {
    const { styles, scripts } = this.getPluginAssets();
    const { output } = this.conf;
    const indexFile = path.resolve(output, './index.html');
    const indexText = (await utils.readFile(indexFile))
      .replace('</head>', styles).replace('</body>', scripts);
  ...
  async build() {
    ...
    await utils.batchExec(locales, item => this.parseLocale(item));
