...
class Generator extends EventEmitter {
  ...
  async readConf() {
    ...
    this.emit('readConf', this);
  };

  async createParser() {
    const options = { html: true, linkify: true };
    this.emit('beforeCreateParser', options, this);
    const md = new MarkdownIt(options);
    this.render = (...args) => {
      this.emit('beforeRender', ...args, this);
      const result = md.render(...args);
      const afterEvent = { result };
      this.emit('afterRender', afterEvent, this);
      return afterEvent.result;
    };
    this.emit('afterCreateParser', md, this);
  }
  ...
  async loadPlugins() {
    ...
    this.emit('loadPlugins', this);
  }

  /**
   * 解析单个文档
   */
  async parseDoc(docFile) {
    let source, meta;
    try {
      source = await utils.readFile(docFile);
    } catch (err) {
    ...
    const item = { source };
    if (meta) Object.assign(item, yaml.safeLoad(meta));
    this.emit('beforeParse', item);
    item.result = await this.render(item.source);
    this.emit('afterParse', item);
    this.log('finished:', docFile);
    return item;
  };
