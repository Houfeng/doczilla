...
class Generator extends EventEmitter {
  ...
  async injectPluginAsstes() {
    if (this.pluginAssetsInjected) return;
    ...
    this.pluginAssetsInjected = true;
  }
  ...
  async build() {
    ...
    await this.writeData({ locales: this.sortDocs(locales) });
    this.info('Done');
  }
  ...
  clearCache(filename) {
    ...
    if (!map) return;
    Object.keys(map).forEach(file => this.clearCache(file));
  }
  ...
  async watch() {
    ...
    watcher.on('all', async (action, filename) => {
      ...
      if (path.extname(filename) !== '.md' && !this.dependents[filename]) {
        return;
      }
